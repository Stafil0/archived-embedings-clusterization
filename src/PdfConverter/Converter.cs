...
namespace MTA.PdfConverter
{
  public static class Converter
  {
    ...
    private static readonly char[] TrimSymbols = { ',', '.', ' ', '?', '!', '-', 'Ч' };
    private static readonly string MatchesRegex = $@"((?:(?(?=(\b{string.Join(@"\.|\b", Inlines)}\.|[^а-€ј-яa-zA-Z][а-€ј-яa-zA-Z]\.\s?|\d+\.\d))\2|[^.!?])+[.!?]))";
    private const string PunctuationRegex = @"[^а-€ј-яa-zA-Z]{2,}";
    private const string ShortWordsRegex = @"\b[а-€ј-яa-zA-Z]{1,2}\b";
